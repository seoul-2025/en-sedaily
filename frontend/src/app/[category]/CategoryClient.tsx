...
import { AdSidebar } from '@/components/ads/AdBanner';
...
function HeroImage({ src, alt, category }: { src: string; alt: string; category?: string }) {
  ...
  if (hasError) {
    return (
      <div className="w-full aspect-[3/2] bg-white flex items-center justify-center rounded-lg mb-6 border border-gray-100">
        <Image
  ...
  return (
    <div className="relative w-full aspect-[3/2] rounded-lg overflow-hidden mb-6 mt-2">
      <Image
        src={src}
        alt={seoAlt}
        title={alt}
...
function ThumbnailImage({ src, alt, category }: { src: string; alt: string; category?: string }) {
  ...
  return (
    <div className="relative w-24 h-16 md:w-32 md:h-20 flex-shrink-0 rounded-lg overflow-hidden">
      <Image
        src={src}
        alt={seoAlt}
        title={alt}
...
function ThumbnailPlaceholder() {
  return (
    <div className="w-24 h-16 md:w-32 md:h-20 flex-shrink-0 bg-white flex items-center justify-center rounded-lg border border-gray-100">
      <Image
        src="/sedaily-og-image.png"
        alt="Seoul Economic Daily - English News from South Korea"
        width={96}
        ...
        {heroArticle && (() => {
          ...
          return (
            <article className="border-b border-[var(--color-border)] pb-8">
              ...
              <h2 className="font-serif text-xl md:text-4xl font-bold leading-tight mb-4 text-[var(--color-primary)] group-hover:text-[var(--color-accent)] transition-colors">
                {heroArticle.title}
              </h2>
            ...
            return (
              <article key={article.news_id} className="border-b border-[var(--color-border)] pb-6 last:border-0">
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="font-serif text-xs md:text-xl font-bold leading-tight mb-2 text-[var(--color-primary)] group-hover:text-[var(--color-accent)] transition-colors line-clamp-2">
                    {article.title}
                  </h3>
                  ...
                  <div className="flex items-center gap-2 text-xs text-[var(--color-text-muted)]">
                    ...
                    <span>
                      {article.byline
                        ? convertByline(article.byline, article.byline_en).replace('By ', '')
