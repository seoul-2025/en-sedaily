...
resource "aws_api_gateway_deployment" "main" {
  depends_on = [
    ...
    aws_api_gateway_integration.cms_delete_post,
  ]
  ...
  triggers = {
    redeployment = sha1(jsonencode([
      ...
      aws_api_gateway_resource.delete_article.id,
      aws_api_gateway_method.cms_delete_post.id,
      aws_api_gateway_integration.cms_delete_post.id,
    ]))
  }
...
# CMS Delete Article POST method
resource "aws_api_gateway_method" "cms_delete_post" {
  rest_api_id   = aws_api_gateway_rest_api.main.id
...
# CMS Delete Article POST integration
resource "aws_api_gateway_integration" "cms_delete_post" {
  rest_api_id = aws_api_gateway_rest_api.main.id
  resource_id = aws_api_gateway_resource.delete_article.id
  http_method = aws_api_gateway_method.cms_delete_post.http_method
...
resource "aws_lambda_permission" "api_gateway_cms_delete" {
  statement_id  = "AllowExecutionFromAPIGateway"
  action        = "lambda:InvokeFunction"
